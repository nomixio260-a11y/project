...
## 操作モード（映像ストリーミング＝リモートブラウザ）

スナップショット方式は**1回だけJSを実行した静止HTML**なので、URLが変わらないSPA内の操作や
動画再生ができません。そこで**サーバー上のブラウザ画面そのものを映像として配信し、
クリック/入力/スクロールを送り返して操作する**リモートブラウザを実装しています。

```
ブラウザ画面 → CDPスクリーンキャスト(JPEGフレーム) → WS → クライアントに描画
クリック/キー/ホイール → WS → CDP Input → 実ページに反映（動画再生も可）
```

- **使い方**: UIの**「操作モード」トグル**。アドレスバーで移動すると、iframeの代わりに
  サーバー側ブラウザの映像が表示され、**動画再生を含む完全な操作**ができます。
- **双方向プロトコル**（WS `/stream`）: サーバー→ `{t:"frame",data}` / `{t:"ready",vw,vh,url}` /
  `{t:"url",url}`、クライアント→ マウス/ホイール/キー/ナビゲーションのJSONイベント。
  クライアントは表示画像上の座標をサーバーのビューポート座標へ変換して送る。
- **省データ寄りの調整**: JPEG品質（`STREAM_QUALITY`）とフレーム間引き（`STREAM_EVERY_NTH_FRAME`）、
  ビューポート上限で転送量を調整可能。マウス移動は約25fpsに間引いて送信。
- **セキュリティ/資源**: スクリーンキャストのブラウザも `newGuardedContext` の `context.route` で
  全リクエストをSSRF検証・重リソース遮断。ナビゲーションURLは毎回再検証。受信入力は型・座標を
  検証し、キー名はホワイトリスト化（注入防止）。同時ストリーム数を `MAX_LIVE_SESSIONS` で制限し、
  WS切断・サーバー終了時に常駐ページを確実に破棄。
- **注意**: スクリーンキャストは**映像のみで音声は含みません**。また映像転送のため、静的処理や
  スナップショット方式に比べ通信量は大きくなります（その代わり完全な操作・動画再生が可能）。

> SPAを「見るだけ」なら省データな**SPA表示**（スナップショット描画）、動画再生や完全な操作が必要なら
> **操作モード**（映像ストリーミング）、と用途で使い分けます。
...
| GET | `/browse?url=&text=0\|1&dw=&dpr=&render=auto\|on\|off` | ページ取得・加工・圧縮して返す（`render`はSPA描画モード） |
| WS | `/stream?url=&dw=&dpr=` | 操作モード: ブラウザ画面を映像配信し、入力を送り返すリモートブラウザ |
...
| `ENABLE_LIVE_SESSIONS` | 1 | `0`で操作モード（映像ストリーミング）を無効化 |
| `MAX_LIVE_SESSIONS` | 4 | 同時に張れる映像ストリームの最大数 |
| `STREAM_QUALITY` | 55 | スクリーンキャストのJPEG品質(0-100、下げるほど省データ) |
| `STREAM_EVERY_NTH_FRAME` | 2 | Nフレームに1枚送る間引き(大きいほど省データ・低fps) |
| `STREAM_MAX_WIDTH` / `STREAM_MAX_HEIGHT` | 1280 | ストリームのビューポート上限(px) |
...
- `screencast` — 操作モードのスクリーンキャスト配信とマウス入力の実ページ反映（Chromium導入時のみ実行）
